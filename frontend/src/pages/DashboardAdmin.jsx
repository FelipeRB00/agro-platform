...
const [errorModal, setErrorModal] = useState('')
  ...
  const statCards = stats ? [
    { icon: 'group', label: 'Total Usuarios', value: stats.total_usuarios, bg: 'bg-secondary-container', text: 'text-on-secondary-container', trend: null },
    { icon: 'storefront', label: 'Proveedores', value: stats.total_proveedores, bg: 'bg-gray-100', text: 'text-primary', trend: null },
    { icon: 'agriculture', label: 'Agricultores', value: stats.total_agricultores, bg: 'bg-green-100', text: 'text-primary', trend: null },
        ...
        <div className="p-8 max-w-7xl w-full mx-auto flex flex-col gap-6">
          ...
          <div className="bg-white rounded-2xl border border-[#dfe7da] overflow-hidden">
            <div className="p-6 border-b border-[#dfe7da] flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <h3 className="text-lg font-bold text-on-surface">Gestión de Usuarios</h3>
              <div className="flex gap-3 flex-wrap">
                {/* Filtro rol */}
                <select value={filtroRol} onChange={e => setFiltroRol(e.target.value)}
                  className="px-3 py-2 border border-[#dfe7da] rounded-lg text-sm text-on-surface focus:outline-none focus:border-secondary bg-white">
    ...
    <div className="bg-white w-full max-w-lg rounded-xl shadow-xl overflow-hidden border border-outline-variant/30">
      ...
      <form onSubmit={handleCrearUsuario}>
        <div className="p-6 space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="text" required placeholder="Juan Pérez"
                ...
                className="w-full pl-9 pr-4 py-2.5 border border-outline-variant rounded-lg text-sm focus:outline-none focus:border-secondary transition-all" />
            </div>
          </div>
          ...
          {/* Rol */}
          <div>
            <label className="block text-xs font-semibold text-on-surface-variant mb-1">Rol del usuario</label>
            <div className="relative">
              <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-outline text-sm">manage_accounts</span>
              <select value={formUsuario.rol}
                onChange={e => setFormUsuario({ ...formUsuario, rol: e.target.value })}
                className="w-full pl-9 pr-4 py-2.5 border border-outline-variant rounded-lg text-sm focus:outline-none focus:border-secondary appearance-none bg-white">
        ...
        <div className="px-6 py-4 border-t border-outline-variant/30 bg-gray-50 flex justify-end gap-3">
          ...
          <button type="submit" disabled={creando}
            className="px-5 py-2 rounded-lg text-sm font-semibold bg-primary text-white hover:bg-primary/90 transition-colors disabled:opacity-60 flex items-center gap-2">
